...
if (isset($_POST["name"]) && isset($_POST["email"]) && isset($_POST["mobile"]) && isset($_POST["password"]) && isset($_POST["confirm_pass"]) && isset($_POST["gender"]) && isset($_POST['User_type'])) {
	...
	if (!$nameerr && !$emailerr && !$mobilerr && !$location_error && !$position_error && !$role_error && !$passworderr && !$confirm_pass_err) {
		$sql = "INSERT INTO `employees` (emp_name, emp_mobile, emp_email, emp_gender, emp_country, emp_state, emp_status, emp_type, emp_role_id, emp_password, emp_created_at, emp_updated_at) VALUES ('$name', '$mobile', '$email', '$gender', $country, $state, '$status', '$position', $role, '$hashed_password', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
		$result = mysqli_query($conn, $sql);
		if ($result) {
			$temp_slug = 'uber_user_added';
			...
			echo '<meta http-equiv="refresh" content="0;url=emp_dashboard.php">';
...
<body>
			...
			<div class="right_side_content">
				...
				<div class="list-contet">
					...
					<form id="main" class="form-edit" onsubmit="validateForm(); return false;"
						action="emp_create.php" method="POST">
						<!-- First Name -->
						<div class="form-row">
	...
	<script src="js/emp_create.js"></script>
</body>
