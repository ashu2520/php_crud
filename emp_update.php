	...
	if (!empty($id) && is_numeric($id)) {
		...
		if ($result) {
			if (mysqli_num_rows($result) > 0) {
				...
				$position = $row['emp_type'];
				$role = $row['emp_role_id'];
				// echo $role;
				if ($role == 1) {
...
if (isset($_POST["Submitasd"])) {
	...
	if (!$error) {
		$sql_1 = "UPDATE `employees` set emp_name='$name', emp_mobile='$mobile', emp_gender= '$gender', emp_country = '$country', emp_state = '$state', emp_status = '$status', emp_type ='$position',  emp_role_id  =$role, emp_updated_at=CURRENT_TIMESTAMP where emp_id = $id";
		$result_1 = mysqli_query($conn, $sql_1);
		if ($result_1) {
			$_SESSION['flash_message'] = "Sucessfully Updated Employee Details";
			...
			<div class="right_side_content">
				...
				<div class="list-contet">
					...
					<form id="main" class="form-edit" onsubmit="return validateForm()"
						action="emp_update.php?updateid=<?php echo $id ?>&column_name=<?php echo $column_name ?>&sort_order=<?php echo $sort_order ?>&page=<?php echo $curr_page ?>"
						method="POST">
